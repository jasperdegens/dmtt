...
test("checkin burned-rung shred: only ladder[L-1] cleared, every other capsule intact", async () => {
  const seed = seededSwitch(5); // advancing FROM liveIdx 5 burns rung 5 = ladder[4]
  const store = makeStore(seed);
  const ctx = makeCtx({ store });

  const res = await checkin(
    ctx,
    { topicId: TOPIC_ID, seq: 4, signal: expectedCheckinSignal(seed) },
    { proof: WORLD_PROOF, action: "check-in" },
  );
  assert.ok(res.ok);
  ...
  assert.ok(sw);
  if (!sw) return;
  assert.equal(sw.ladder[4].capsuleB64, "", "burned rung 5 (ladder[4]) shredded");
  // EVERY other rung untouched.
  sw.ladder.forEach((r, i) => {
    if (i !== 4) assert.equal(r.capsuleB64, `CAPSULE_${i + 1}`, `rung ${i + 1} intact`);
  });
});

test("checkin multi-walk on a 4-rung ladder: seq 0→1→2→3, liveIdx===seq+1, each burns its rung, final → LADDER_EXHAUSTED unchanged", async () => {
  const n = 4;
  const seed = seededSwitch(1, n); // liveIdx 1, seq 0
  const store = makeStore(seed);
  const { hedera, log } = makeHedera();
  const ctx = makeCtx({ store, hedera });

  // Three successful check-ins: from liveIdx 1→2→3→4 (seq 0→1→2→3).
  for (let step = 0; step < n - 1; step++) {
    const before = await store.load(TOPIC_ID);
    assert.ok(before);
    if (!before) return;

    const res = await checkin(
      ctx,
      { topicId: TOPIC_ID, seq: before.seq, signal: expectedCheckinSignal(before) },
      { proof: WORLD_PROOF, action: "check-in" },
    );
    assert.ok(res.ok, `check-in ${step} should succeed`);
    if (!res.ok) return;

    const after = await store.load(TOPIC_ID);
    assert.ok(after);
    if (!after) return;

    assert.equal(after.seq, step + 1, "seq advances by one");
    assert.equal(after.liveIdx, after.seq + 1, "invariant liveIdx===seq+1");
    // The rung just burned is ladder[before.liveIdx - 1].
    assert.equal(after.ladder[before.liveIdx - 1].capsuleB64, "", "the soonest rung was burned");
    // The new live rung still has its capsule.
    assert.equal(after.ladder[after.liveIdx - 1].capsuleB64, `CAPSULE_${after.liveIdx}`);
  }

  // Now liveIdx === n (=4), seq === n-1 (=3): no rung to advance to.
  const exhausted = await store.load(TOPIC_ID);
  assert.ok(exhausted);
  if (!exhausted) return;
  assert.equal(exhausted.liveIdx, n);
  assert.equal(exhausted.seq, n - 1);
  ...
  assert.deepEqual(await store.load(TOPIC_ID), snapshot, "store unchanged on exhaustion");
  assert.equal(log.calls.length, callsBefore, "no further hedera calls");
});
