...
- **[G0] Cost (measured): HFS ≈ 1 ℏ/KB, FLAT** — shortening file expiration does NOT reduce it (2 KB cost 2.09 ℏ at 1-day ≈ 2 ℏ at 90-day). So the >4 KB path is costly (1 MB ≈ ~1000 ℏ) — **prefer the ≤4 KB fast path**. HCS messages cost ≈ 0.01 ℏ/KB (**~100× cheaper**, mirror-served, no `FileContentsQuery` proxy). **[Phase 2 — FROZEN]** the freeze resolved this: `StorageRef` is a discriminated union — the ≤4 KB **HFS fast path stays canonical/default**; ciphertext >4 KB (up to ~1 MB scope) uses the **HCS large path** (chunked on a dedicated topic, reassembled from the mirror). This amends C2 by *adding* the HCS medium; the create→append→seal path stays valid but is deprecated for large files on cost grounds. See `docs/CONTRACTS.md` §2.
