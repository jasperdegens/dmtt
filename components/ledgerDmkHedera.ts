...
// Ethereum/Bitcoin/Solana/Cosmos/… but NOT Hedera (see docs/LEDGER_FEEDBACK.md, Gap 2).
...
const USER_MESSAGE: Record<LedgerErrorKind, string> = {
  ...
  timeout: "Timed out waiting for the Ledger. Reconnect the device and try again.",
  transport: "Lost the connection to the Ledger. Reconnect the device and try again.",
  unknown: "The Ledger returned an unexpected error. Reconnect the device and try again.",
};
...
export function toLedgerError(err: unknown): LedgerError {
  ...
  if (e?.ledgerKind) return new LedgerError(e.ledgerKind, USER_MESSAGE[e.ledgerKind], e.message);

  const tag = e?._tag ?? "";
  const code = e?.errorCode ?? e?.originalError?.errorCode ?? "";

  let kind: LedgerErrorKind = "unknown";
  if (tag === "RefusedByUserDAError" || code === "5501" || code === "6985") kind = "rejected";
  else if (tag === "DeviceLockedError" || code === "5515" || code === "6982" || code === "5303")
    kind = "locked";
...
function statusWordError(response: ApduResponse): InvalidStatusWordError & {
  ...
  ) as InvalidStatusWordError & { ledgerKind: LedgerErrorKind; statusWord: number };
  err.ledgerKind = kind;
  err.statusWord = sw;
  return err;
}
...
class HederaSignTransactionCommand implements Command<Uint8Array> {
  ...
  constructor(
    ...
    if (body.length > MAX_BODY_BYTES) {
      throw new Error(`transaction body too large for one APDU (${body.length} > ${MAX_BODY_BYTES})`);
    }
  }

  getApdu() {
    return new ApduBuilder({ cla: CLA, ins: INS_SIGN_TRANSACTION, p1: 0x00, p2: 0x00 })
...
export function isWebHidSupported(): boolean {
  return typeof navigator !== "undefined" && Boolean((navigator as Navigator & { hid?: unknown }).hid);
}
...
export async function openHederaApp(
  ...
  const { observable, cancel } = dmk.executeDeviceAction({
    sessionId,
    deviceAction: new OpenAppDeviceAction({ input: { appName: HEDERA_APP_NAME } }),
  });
      ...
      next: (state) => {
        switch (state.status) {
          ...
          case DeviceActionStatus.Stopped:
            done(() => reject(new LedgerError("rejected", USER_MESSAGE.rejected)));
            break;
...
export async function getHederaPublicKey(
  ...
  const result = await dmk.sendCommand({
    sessionId,
    command: new HederaGetPublicKeyCommand(opts.index ?? DEFAULT_KEY_INDEX, opts.display ?? false),
  });
...
export async function signHederaTransactionBody(
  ...
  const result = await dmk.sendCommand({
    sessionId,
    command: new HederaSignTransactionCommand(opts.index ?? DEFAULT_KEY_INDEX, body),
  });
  if (!isSuccessCommandResult(result)) throw toLedgerError(result.error);
  return result.data;
}
